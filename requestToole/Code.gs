...
const ANSWER_COLUMN_NAMES = ANSWER_SHEET.getRange(1, 1, 1, 69).getValues().flat();
...
function onOpen() {
	...
	ui.createMenu('✈️ XBOM').addItem('✍️ Correct items', 'correctPrevious').addToUi();
}
...
function correctPrevious() {
	var ui = SpreadsheetApp.getUi();
	var response = ui.prompt('It Should be used in case of bug', 'is there any bug?', ui.ButtonSet.OK_CANCEL);
    ...
    console.log(answerSheetLastRow)
		for (var indexRow = lastRowXbomId; indexRow <= answerSheetLastRow; indexRow++) {
...
function registerNecessaryInfo(indexRow, sendMessage = true) {
	...
	var titleAnswer = ANSWER_SHEET.getRange(answerRowIndex, TITLE_COLUMN + 1).getValue();

	if (regExpSubtask.test(titleAnswer)) {
		xbomIdNumber = regExpSubtask.exec(titleAnswer)[0].replace('[', '').replace(']', '');
		titleAnswer = titleAnswer.substring(('[' + xbomIdNumber + '] ').length, titleAnswer.length);
	} else {
		var lastRowXbomId = getLastRowFromColumn(ANSWER_SHEET, ID_XBOM_COLUMN + 1);
		var allXbomId = ANSWER_SHEET.getRange(1, ID_XBOM_COLUMN + 1, lastRowXbomId, 1)
			.getValues()
			.flat();
		...
		xbomIdNumber = 'XBOM-' + (parseInt(regExpLastDigits.exec(lastXbomIdNumber)[0]) + 1);
	}

	ANSWER_SHEET.getRange(answerRowIndex, ID_XBOM_COLUMN + 1)
		.setValue(xbomIdNumber)
		.setHorizontalAlignment('center');
	ANSWER_SHEET.getRange(answerRowIndex, ANSWER_COLUMN_NAMES.indexOf('Stream') + 1).setValue('Waiting stream');
	ANSWER_SHEET.getRange(answerRowIndex, ANSWER_COLUMN_NAMES.indexOf('Topic') + 1).setValue('Waiting topic');
	ANSWER_SHEET.getRange(answerRowIndex, ANSWER_COLUMN_NAMES.indexOf('Status') + 1).setValue('To affect');
	ANSWER_SHEET.getRange(answerRowIndex, ANSWER_COLUMN_NAMES.indexOf('Référent') + 1).setValue('Waiting assignment');
	ANSWER_SHEET.getRange(answerRowIndex, ANSWER_COLUMN_NAMES.indexOf('XBOM Assignment') + 1).setValue('Waiting assignment');
	ANSWER_SHEET.getRange(answerRowIndex, ANSWER_COLUMN_NAMES.indexOf('Progress status') + 1).setValue('0%');
	ANSWER_SHEET.getRange(answerRowIndex, ANSWER_COLUMN_NAMES.indexOf('Workload') + 1).setValue('Waiting level');
	...
	messageToSendInChat =
		'```🆕 <users/all> \n\nA new item [' + xbomIdNumber + '] - ' + titleAnswer + '\nfrom: ' + ANSWER_SHEET.getRange(answerRowIndex, REQUESTOR_COLUMN + 1).getValue() + ' has been added \n=========================================```';
	try {
		sendMessage ? notifyViaChat(messageToSendInChat, WEBHOOK_REQUEST_CHAT) : Logger.log('pass');
	} catch {
		Logger.log('pass');
	}
}
...
function onEdit(e) {
  ...
  if ((ANSWER_SHEET.getRange(editRowIndex, 1).getValue() == '') || (newValue == oldValue && newValue != null)){ return }
	if (e.range.getRowIndex() == 1) {
		if (e.range.getNumRows() > 1 || e.range.getNumColumns() > 1) {
			ANSWER_SHEET.getRange(1, 1, 1, ANSWER_SHEET.getLastColumn()).setValues(FEEDER_SHEET.getRange(1, 2, 1, ANSWER_SHEET.getLastColumn()).getValues());
			try {
	...
	if (editRowIndex == 1) {
		...
		try {
			var response = ui.alert('Are you sure?', `Are you sure you want to edit the header column?\n\n  Your want to rename [${oldValue}] to → [${newValue}] \n\n`, ui.ButtonSet.OK_CANCEL);
			if (response == ui.Button.CANCEL) {
				ANSWER_SHEET.getRange(1, e.range.getColumn()).setValue(oldValue);
			} else if (response == ui.Button.OK) {
				try {
					var response2 = ui.alert('Be careful !', `This will impact DATASTUDIO & APP SCRIPTS`, ui.ButtonSet.OK_CANCEL);
					if (response2 == ui.Button.CANCEL) {
						ANSWER_SHEET.getRange(1, e.range.getColumn()).setValue(oldValue);
					} else if (response2 == ui.Button.OK) {
						FEEDER_SHEET.getRange(1, e.range.getColumn() + 1).setValue(oldValue);
						messageToSendInChat = '```🚨 <users/all>\n\n' + Session.getActiveUser().getEmail() + '\nhas changed the column: [' + oldValue + '] to → [' + newValue + '] ```';
  ...
  ANSWER_SHEET.getRange(editRowIndex, ANSWER_COLUMN_NAMES.indexOf('LAST PERSON TO UPDATE') + 1, e.range.getNumRows(), 1).setValue(Session.getActiveUser().getEmail());
  ANSWER_SHEET.getRange(editRowIndex, ANSWER_COLUMN_NAMES.indexOf('LAST UPDATE') + 1, e.range.getNumRows(), 1).setValue(new Date());

	if (newValues.flat().includes('Done')) {
		ANSWER_SHEET.getRange(editRowIndex, ANSWER_COLUMN_NAMES.indexOf('Progress status') + 1, e.range.getNumRows(), 1).setValue('100%');
		if (ANSWER_SHEET.getRange(editRowIndex, ANSWER_COLUMN_NAMES.indexOf('Closing Date ') + 1, e.range.getNumRows(), 1).getValue() == '') {
			ANSWER_SHEET.getRange(editRowIndex, ANSWER_COLUMN_NAMES.indexOf('Closing Date ') + 1, e.range.getNumRows(), 1).setValue(new Date());
		}
	}
